...
var (
	Middleware = middleware.NewMiddleware("tunnel", setRoutes, middlewareFunc)
)

func setRoutes(r *mux.Router) {
	r.PathPrefix("/tunnel={address}").Subrouter().MatcherFunc(func(*http.Request, *mux.RouteMatch) bool { return true }).HandlerFunc(tunnel)
	...
	util.AddRoute(tunnelRouter, "/add/{endpoint}/transparent", addTunnel, "POST", "PUT")
	util.AddRouteQO(tunnelRouter, "/remove/{endpoint}/header/{header}={value}", clearTunnel, "uri", "POST", "PUT")
	util.AddRouteQO(tunnelRouter, "/remove/{endpoint}/header/{header}", clearTunnel, "uri", "POST", "PUT")
	util.AddRouteQO(tunnelRouter, "/remove/{endpoint}", clearTunnel, "uri", "POST", "PUT")
	util.AddRoute(tunnelRouter, "/clear", clearTunnel, "POST", "PUT")

	util.AddRoute(tunnelRouter, "/track/header/{headers}", addTunnelTracking, "POST", "PUT")
	util.AddRoute(tunnelRouter, "/track/query/{params}", addTunnelTracking, "POST", "PUT")
	util.AddRoute(tunnelRouter, "/track/clear", clearTunnelTracking, "POST", "PUT")
	util.AddRoute(tunnelRouter, "/track", getTunnelTracking, "GET")

	util.AddRoute(tunnelRouter, "/traffic/capture={yn}", captureTunnelTraffic, "POST", "PUT")
	util.AddRoute(tunnelRouter, "/traffic", getTunnelTrafficLog, "GET")

	util.AddRoute(tunnelRouter, "/active", getActiveTunnels, "GET")
	util.AddRoute(tunnelRouter, "", getTunnels, "GET")
}
