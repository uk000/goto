...
func (l *Listener) initListener() bool {
  ...
  if l.AutoCert {
    ...
    if cert, err := util.CreateCertificate(l.CommonName); err == nil {
      l.Cert = cert
    }
  }
  if l.Cert != nil {
...
func (l *Listener) openListener() bool {
  if l.initListener() {
    ...
    l.Open = true
    l.TLS = len(l.RawCert) > 0 && len(l.RawKey) > 0
    return true
  }
  return false
}
...
func addOrUpdateListener(l *Listener, update bool) (int, string) {
  ...
  if isHTTPS || strings.EqualFold(l.Protocol, "http") {
    ...
    l.isTCP = true
    if strings.EqualFold(l.Protocol, "tls") {
      l.Protocol = "tcp"
      l.TLS = true
      if l.Cert == nil && l.RawCert == nil {
        l.AutoCert = true
...
func autoCert(w http.ResponseWriter, r *http.Request) {
  if l := validateListener(w, r); l != nil {
    ...
    if domain := util.GetStringParamValue(r, "domain"); domain != "" {
      if cert, err := util.CreateCertificate(domain); err == nil {
        l.Cert = cert
        if l.reopenListener() {
